...
First thing I had to do was transforming the map. The little fields placed on the street were made with blender. For the textures I created a low poly color map for all my components with gimp. I used a combination of blue and orange, since I think these colors harmonize well with each other and with the templete we got. I extended the map for creating more complex textures for the minigames. There is also space for further extending.
...
After creating the fields I placed them in the map in relatively equal distances. I consider different numbers of total fields and found that 45 was a good number, that you have to perform a some minigames but not too much since it should only be a small game. Every field got an collider and an tag named field so I can track if the player is standing on a field. The fields are placed in the correct order so I can get the correct position of the current and the next field by tracking how many jumps the player ahd performed. I placed the coins at the highest jump point. Thus the player would collect all coins without doing anything special. I thought of placing them a little bit off place to that the coins had to be pushed by the controllers while jumping, but I didn't know if it would be a little bit too fast with first swinging swinging the arms to jump and then immediatly pushing them to reach to the coin. Therefor I stayed with the simple approach of collecting all coins.
